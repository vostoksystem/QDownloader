...
bool QDownloaderService::start(QString const & source, QString const & target, QString const &message ) {
    ...
    if(d.data()->start(source,target,message) == false) {
        ...
        if( signalOnRootError ) {
            emit error(QUrl(source), QNetworkReply::OperationCanceledError);
        }
        return false;
    }
    ...
    connect(d.data(), SIGNAL(error(QUrl,QNetworkReply::NetworkError)), this, SLOT(errorImpl(QUrl,QNetworkReply::NetworkError)));
...
#ifdef QT_DEBUG
    qDebug() << "QDownloader ; success : " << url;
#endif
 ...
 * @param status
 */
void QDownloaderService::errorImpl(QUrl const & url, QNetworkReply::NetworkError status ) {
#ifdef QT_DEBUG
    qDebug() << "QDownloader ERROR : " << url;
#endif

    pending.remove(url.toString());
    emit error(url, status);
    emit pendingChanged();
}
